...
with tab1:
    subtab1, subtab2, subtab3, subtab4, subtab5, subtab6 = st.tabs(
        ["연령대별 고객 분포", "지역별 고객 분포", "연령대별 선호 차량 모델", 
         "연령대별 친환경 차량 선호도", "성별 및 연령대별 차량 선호도", "연령대별 고객 등급"]
    )
    ...
    with subtab4:
        ...
        # 고객 등급 분석 (도넛 차트)
        with subtab6:
            # 연령대별 고객 등급 분포 데이터 생성
            grade_by_age = df.groupby(['연령대', '고객 등급']).size().reset_index(name='count')
            grade_by_age["연령대"] = pd.Categorical(grade_by_age["연령대"], categories=age_order, ordered=True)
            grade_by_age = grade_by_age.sort_values("연령대")

            # Plotly Express를 사용한 스택형 막대 차트 생성
            fig = px.bar(
                grade_by_age, 
                x='연령대', 
                y='count', 
...
# 판매 데이터 분석 탭
with tab2:
    subtab1, subtab2 = st.tabs([ "시기 및 연료 구분별 판매 대수", "분기별 차량 판매 요일"])
